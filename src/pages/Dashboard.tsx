...
export default function Dashboard() {
  ...
  if (loading) return <div className="p-8 animate-pulse">Loading archive...</div>;
  if (error) return (
  ...
  const stats = [
    { label: 'Total Items', value: items.length, icon: Package, color: 'text-blue-500', bg: 'bg-blue-50' },
    { label: 'Locations', value: homes.length, icon: HomeIcon, color: 'text-purple-500', bg: 'bg-purple-50' },
    { label: 'Storage Units', value: storages.length, icon: Warehouse, color: 'text-amber-500', bg: 'bg-amber-50' },
  ];
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex items-center justify-between">
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-zinc-50">
            ...
            {items.length === 0 && (
              <div className="p-12 text-center text-zinc-400 text-sm">No items archived yet.</div>
            )}
          </div>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6">
          ...
          <div className="space-y-6">
              ...
              return (
                <div key={home.id} className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-zinc-500">{homeItems.length} items</span>
                  </div>
            ...
            {homes.length === 0 && (
              <div className="py-12 text-center text-zinc-400 text-sm">No homes defined in settings.</div>
            )}
          </div>
        </section>
      </div>
